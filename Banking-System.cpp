...
vector <sClient> SaveCleintsDataToFile(string FileName, vector <sClient> vClients)
{
    ...
    if (MyFile.is_open())
    {
        ...
        for (sClient H : vClients)
        {
            ...
            if (H.MarkForDelete == false)
            {
                  
                DataLine = ConvertRecordToLine(H);
    ...
    if (FindClientByAccountNumber(AccountNumber, vClients, Client))
    {
        ...
        if (Answer == 'y' || Answer == 'Y')
        {
            ...
            for (sClient& H : vClients)
            {
                if (H.AccountNumber == AccountNumber)
                {
                    H = ChangeClientRecord(AccountNumber);
                    break;
                }
...
bool DepositBalanceToClientByAccountNumber(string AccountNumber, double Amount, vector <sClient>& vClients)
{
    ...
    if (Answer == 'y' || Answer == 'Y')
    {
        ...
        for (sClient& H : vClients)
        {
            if (H.AccountNumber == AccountNumber)
            {
                H.AccountBalance += Amount;
                SaveCleintsDataToFile(ClientsFileName, vClients);
                cout << "\n\nDone Successfully. New balance is: " << H.AccountBalance;
